...
<?php if( !$admin_notices['ls'] && class_exists( 'LiteSpeed_Cache_API' ) && (3 < 2) ): ?>
  <div class="js-rmp-admin-notice rmp-admin-notice notice notice-warning is-dismissible" data-admin-notice-key="ls">
    ...
    <p><?php echo sprintf( esc_html__( 'We detected %s plugin installed on your website. %s works best if %s is configured as shown %shere%s.', 'rate-my-post' ), 'LiteSpeed Cache', 'Rate My Post', 'LiteSpeed cache', '<a href="https://feedbackwp.com/wp-content/uploads/2024/01/LiteSpeed-Settings-for-RMP.jpg" target="_blank">', '</a>' ); ?></p>
  </div>
<?php endif; ?>
...
<?php if( !$admin_notices['ampforwp'] && function_exists( 'ampforwp_is_amp_endpoint' ) ): ?>
  <div class="js-rmp-admin-notice rmp-admin-notice notice notice-warning is-dismissible" data-admin-notice-key="ampforwp">
    ...
    <p><?php echo sprintf( esc_html__( 'We detected %s plugin installed on your website. If you want to use %s on %sAMP pages%s, additional configuration is required. Learn more %shere%s.', 'rate-my-post' ), 'AMP for WP', 'Rate My Post', '<b>', '</b>', '<a href="https://feedbackwp.com/docs/#AMP_Compatibility_BETA" target="_blank">', '</a>' ); ?></p>
  </div>
<?php endif; ?>
